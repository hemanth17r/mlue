...
export default function Hero({ latestRun }) {
  ...
  const rawSpeed = b.find((item) => item.id === 'B6')?.value_display || '11.6k t/s';
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 max-w-3xl mx-auto">
        {metrics.map((m, idx) => (
          <motion.div 
            {...tapScale.card}
            key={idx}
            className="p-3.5 sm:p-4 rounded-2xl bg-slate-900/80 border border-white/[0.08] hover:border-cyan-500/40 text-left shadow-lg transition-colors cursor-default"
          >
            <div className="text-[10px] font-mono uppercase text-slate-400 tracking-wider font-semibold truncate">
              {m.label}
            </div>
            <div className={`text-base sm:text-lg font-bold font-mono ${m.color} mt-1 flex items-baseline gap-1`}>
              <span>{m.value}</span>
              {m.suffix && (
